            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {criticalAlerts.map((alert) => {
                ...
                return (
                  <div
                    key={alert.id}
                    className="bg-white rounded-lg p-3 flex items-center gap-4 shadow-sm border border-yellow-100"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-bold text-[#C62828]">
                        {formatMinutesToHHMM(alert.saldo_total)}
                      </p>
                    </div>
                  </div>
                )
                  ...
                  return (
                    <TableRow key={saldo.id} className="hover:bg-gray-50/50">
                      ...
                      <TableCell className="text-gray-600">
                        {formatMinutesToHHMM(saldo.saldo_anterior)}
                      </TableCell>
                      <TableCell className="text-gray-600">
                        {formatMinutesToHHMM(saldo.saldo_mes)}
                      </TableCell>
                      <TableCell
                        className={`font-bold ${isPositive ? 'text-[#2E7D32]' : isNegative ? 'text-[#C62828]' : 'text-gray-600'}`}
                      >
                        {formatMinutesToHHMM(saldo.saldo_total)}
                      </TableCell>
                      <TableCell>
                        {isPositive ? (
                          <Badge className="bg-[#2E7D32] hover:bg-[#2E7D32]/90 text-white border-transparent">
                            Positivo
                          </Badge>
                        ) : isNegative ? (
                          <Badge className="bg-[#C62828] hover:bg-[#C62828]/90 text-white border-transparent">
                            Negativo
                          </Badge>
                        ) : (
                          <Badge
                            variant="secondary"
                            className="bg-gray-200 text-gray-700 hover:bg-gray-300 border-transparent"
                          >
                            Zerado
                          </Badge>
                        )}
